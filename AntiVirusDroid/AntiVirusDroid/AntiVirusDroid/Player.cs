...
namespace AntiVirusDroid
{
    static class Player
    {
        ...
        private static Vector2 handleMovement()
        {
            
            return  new Vector2 ( -(float)Math.Atan2(
        ...
        private static Vector2 handleShots()
        {
            return new Vector2(-(float)Math.Atan2(
        ...
        private static void handleInput(GameTime gameTime)
        {
            ...
            if (VirtualThumbsticks.LeftThumbstick.Length() > .3f)
            {
                moveAngle += handleMovement();
